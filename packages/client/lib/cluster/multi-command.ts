...
> = (...args: Parameters<C['transformArguments']>) => ValkeyClusterMultiCommandType<M, F, S>;
...
export type ValkeyClusterMultiCommandType<
    ...
    F extends ValkeyFunctions,
    S extends ValkeyScripts
> = ValkeyClusterMultiCommand & WithCommands<M, F, S> & WithModules<M, F, S> & WithFunctions<M, F, S> & WithScripts<M, F, S>;

export type InstantiableValkeyClusterMultiCommandType<
    M extends ValkeyModules,
    F extends ValkeyFunctions,
    S extends ValkeyScripts
> = new (...args: ConstructorParameters<typeof ValkeyClusterMultiCommand>) => ValkeyClusterMultiCommandType<M, F, S>;
...
export default class ValkeyClusterMultiCommand {
    ...
    static extend<
        ...
        F extends ValkeyFunctions,
        S extends ValkeyScripts
    >(extensions?: ValkeyExtensions<M, F, S>): InstantiableValkeyClusterMultiCommandType<M, F, S> {
    ...
    commandsExecutor(command: ValkeyCommand, args: Array<unknown>): this {
        ...
        this.#firstKey ??= ValkeyCluster.extractFirstKey(command, args, transformedArguments);
    ...
    functionsExecutor(fn: ValkeyFunction, args: Array<unknown>, name: string): this {
        ...
        this.#firstKey ??= ValkeyCluster.extractFirstKey(fn, args, transformedArguments);
        return this;
    }

    scriptsExecutor(script: ValkeyScript, args: Array<unknown>): this {
        const transformedArguments = this.#multi.addScript(script, args);
        this.#firstKey ??= ValkeyCluster.extractFirstKey(script, args, transformedArguments);
        return this;
    }
    ...
    async exec(execAsPipeline = false): Promise<Array<ValkeyCommandRawReply>> {
        ...
        return this.#multi.handleExecReplies(
            await this.#executor(this.#multi.queue, this.#firstKey, ValkeyMultiCommand.generateChainId())
        );
    }
